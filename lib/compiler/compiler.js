...
var fs = require('fs'),
	...
	BINARY_FN = (function() {
		...
		Object.keys(map).forEach(function(key) {
			...
			if (inv)
				fName = fName.slice(1);
			fns[key] = function(left, right) {
				if (inv)
					return toProtoCall(fName, [ right, left ]);
				return toProtoCall(fName, [ left, right ]); 
			};
		});
		return fns;
	})();
...
[
	'global', 'undefined', 'null', '$P', 'function', 'instanceof',
...
function compile(program, relPath, _idWhitelist) {
	...
	topScope = currentScope = create(null);
	// TODO: Figure out how to add these to neededRuntimeProps only when necessary.
	topScope['sym_@iterator'] = toProtoProperty('@iterator');
	topScope['sym_@toComparable'] = toProtoProperty('@toComparable');
	module.exports.state = state = create(null);
	...
	processed.body = [
		esgen.createExpressionStatement(
			esgen.createCallExpression(
				esgen.createFunctionExpression(
					null, [ ], [ ],
					esgen.createBlockStatement(processed.body),
					null, false, false
				),
				[ ]
			)
...
function checkIdentifierName(name, scope) {
	if (scope['Identifier:defined_' + name])
		return name;
...
function setupIdentifiers() {
	...
	return {
		reset: function reset() {
			compilationId = ''; //Identifiers.getRandomString(1);
			identifiers.reset();
		},
		next: function next() {
...
function permanentVar(init, name) {
	var pvar;
	if (name === undefined)
		pvar = identifiers.next();
	else
		pvar = esgen.createIdentifier(name);
	programInsert(
		protogen.createVariableDeclaration(
...
function unaryBindOperator(node) {
	...
	var tmp = tmpVar(),
		...
		fn = protogen.createMemberExpression(
...
// but doesn't make it non-writable/non-configurable (should it do both?).
	...
	for (var i = 0; i < node.properties.length; i++) {
		...
		|| prop.kind != 'init'
		|| prop.conditional
		// We need to put potential magical properties in the extended
		// properties array to ensure that  they don't take on their
		// magicalness in the simple properties object.
		|| /^__/.test(prop.key.name || prop.key.value)) {
			...
			if (prop.key.type == 'Literal' && prop.key.kind == 'Symbol')
				...
				key = createStringLiteral(prop.key.value);
			else
				throw new Error(
...
function breakupBinaryOrLogicalExpression(node) {
	...
	switch(operator) {
		...
		case '??':
			rightAssign = protogen._createConditionalExpression(
				esgen.createBinaryExpression('==', tmpL, ES_NULL),
				rightAssign, ES_UNDEFINED
			);
			break;
	}
...
function assignCompletion(identifier, node) {
	...
	case 'LabeledStatement':
		node.body = assignLabeledBreakCompletion(
			node.label, identifier, node.body
		);
	...
	case 'DoWhileStatement':
		...
		node.body = assignBreakCompletion(tmp, node.body);
	...
	case 'BreakStatement':
		return node;
	default:
		// TODO: What about FunctionStatement?
...
function assignBreakCompletion(identifier, node) {
	...
	traverse(node, function(info) {
		...
		switch (n.type) {
		case 'BreakStatement':
			insertAssignmentForCompletion(parent, key, identifier, n);
			return false;
		case 'ForStatement':
		case 'WhileStatement':
		case 'DoWhileStatement':
...
function assignLabeledBreakCompletion(label, identifier, node) {
	traverse(node, function(info) {
		var n = info.node,
			parent = info.parent,
			key = info.key;
		if (!isObject(n))
			return false;
		if (n.type == 'BreakStatement' && n.label.name == label.name) {
			insertAssignmentForCompletion(parent, key, identifier, n);
			return false;
		}
		if (n.type == 'LabeledStatement' && n.label.name == label.name)
			return false;
		return true;
	});
	return node;
...
function insertAssignmentForCompletion(parent, key, identifier, node) {
	var index = -1;
	if (Array.isArray(parent) && key > 0)
		index = findLastNonEmptyStatement(parent, key - 1);
	if (~index)
		parent[index] = toStatement(
			protogen._createAssignmentExpression('=',
				identifier, statementToExpression(parent[index])
			)
		);
	else
		parent[key] = protogen._createBlockStatement([
			esgen.createExpressionStatement(
				esgen.createAssignmentExpression('=',
					identifier, ES_UNDEFINED
				)
...
function findLastNonEmptyStatement(body, upTo) {
	var autoInsert = upTo !== undefined;
	if (upTo === undefined)
		upTo = body.length - 1;
...
function processArrayComprehension(node, scope) {
	var tmp = tmpVar(),
		tmpI = tmpVar();
	scope.comprehensionResult = tmp;
	scope.comprehensionIndex = tmpI;
	return protogen.createSequenceExpression([
		esgen.createAssignmentExpression('=',
			tmp,
			toProtoCall('CreateArray', [ ES_UNDEFINED ])
		),
		esgen.createAssignmentExpression('=',
			tmpI,
			createLiteral(0, '0')
		...
		protogen._createStatementExpression(
			protogen._createBlockStatement(node.body)
		),
		tmp
	]);
}

function processGeneratorComprehension(node, scope) {
	var tmp = tmpVar();
	scope.comprehensionResult = tmp;
...
function addEachValueToComprehension(expression, array, index) {
	var tmp = tmpVar();
	return protogen.createSequenceExpression([
		protogen._createAssignmentExpression('=',
			tmp, expression
		...
		protogen._createAssignmentExpression('=',
			protogen._createMemberExpression(true, true, array,
				protogen._createPostfixExpression('++', index)
			),
			tmp
		),
		protogen._createBinaryExpression('-',
			protogen._createAssignmentExpression('=',
				protogen._createMemberExpression(false, true, array,
					esgen.createIdentifier('length')
				),
				index
			),
			createLiteral(1, '1')
	...
	case '??=':
		return esgen.createAssignmentExpression('=',
			...
			protogen.createBinaryExpression(
				node.operator.slice(0, -1), node.left, node.right
			)
		);
	}
...
function BinaryExpression(node) {
	...
	if (node.operator[0] == '!')
		return protogen.createUnaryExpression('!',
			protogen._createBinaryExpression(
				node.operator.slice(1), node.left, node.right
			)
		);
	switch (node.operator) {
	...
	case 'in':
		...
		// fallthrough
	default:
		fn = BINARY_FN[node.operator];
		if (fn === undefined)
			throw new Error('Unimplemented operator "' + node.operator + '"');
		return fn(process(node.left), process(node.right));
	}
}
...
function CascadeStatement(node, scope) {
	...
	assert(node.block.type == 'BlockStatement');
...
function ComprehensionExpression(node, scope) {
	switch (node.kind) {
	case 'array':
		return processArrayComprehension(node, scope);
	case 'generator':
		return processGeneratorComprehension(node, scope);
	default:
		throw new Error(
			'Unimplemented "' + kind + '" ComprehensionExpression'
		);
	}
}
...
function ForOfStatement(node, scope) {
	...
	assert(u.type == 'Identifier');

	blockInsert(
		protogen.createVariableDeclaration(
			[ protogen._createVariableDeclarator(u, null, null) ],
			'var'
		)
	);
...
function FunctionDeclaration(node, scope) {
	var fe_node = protogen._createFunctionExpression(
			// No id for hoisted FDs because the id is used in the declarator.
			// This makes recursive calls to FDs easy and also preserves JS
			// semantics in situations like the following:
			//     function foo(a) {
		...
		id = esgen.createIdentifier(
			node.id.__ES__
				? node.id.name
				: checkIdentifierName(node.id.name, scope)
		);
	fe_node.fn_name = node.id.name;
	blockHoist(
		protogen.createVariableDeclaration(
			[ protogen._createVariableDeclarator(id, fe_node, null) ],
			'var'
		)
	);
}
...
function FunctionExpression(node, scope) {
	...
	body = process(body, { processingGenerator: generator || async });
...
function Identifier(node, scope, params) {
	...
	if (node.coercive != null)
		throw new SyntaxError('Unexpected coercive');
	var name = node.__ES__ ? node.name : checkIdentifierName(node.name, scope),
		tmp;
	if (!params.id_skipGlobal && !scope['Identifier:defined_' + name]) {
		tmp = permanentVar(
			toProtoCall('GetBuiltInGlobal', [
				createStringLiteral(node.name)
			]),
			name
		);
		topScope['Identifier:defined_' + tmp.name] = true;
		// topScope['Identifier:rename_' + name] = tmp.name;
		return tmp;
	}
...
function LabeledStatement(node) {
	...
	addDefinedVar(label, block, 'label');
...
function UnaryExpression(node, scope) {
	...
	case 'each':
		assert('comprehensionResult' in scope);
		assert('comprehensionIndex' in scope);
		return addEachValueToComprehension(
			node.argument,
			scope.comprehensionResult,
			scope.comprehensionIndex
		);
	default:
		return esgen.createUnaryExpression(
