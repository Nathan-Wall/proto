...
function ComprehensionExpression(node) {
	...
	return protogen.createSequenceExpression([
		...
		protogen._createStatementExpression(
			processComprehensionBlock(node.blocks, node.body, tmp, tmpI)
		),
		tmp
	]);
}

function processComprehensionBlock(blocks, body, tmp, tmpI) {
	blocks = blocks.slice();
	var block = blocks.pop(),
		blockBody;
	if (blocks.length)
		blockBody = processComprehensionBlock(blocks, body, tmp);
	else
		blockBody = protogen._createExpressionStatement(
			protogen._createAssignmentExpression('=',
				protogen._createMemberExpression(true, true, tmp,
					protogen._createPostfixExpression('++', tmpI)
				),
				body
			)
		);
	return protogen._createForOfStatement(
		block.left, block.right, blockBody
	);
}
