...
function isSequenceable(node) {
	if (node.type == 'ExpressionStatement')
		return !containsBlockExpression(node);
...
function bodyToSequenceExpression(body) {
	...
	return protogen.createSequenceExpression(output);

}

function ifStatementToConditionalExpression(node) {
	return protogen._createConditionalExpression(
			node.test,
...
function containsBlockExpression(node) {
	var containsBlock = false;
	traverse(node, function(info) {
		var type = info.node && info.node.type;
		if (type == 'BlockExpression')
			containsBlock = true;
...
function deblockExpression(node) {
	// TODO: Are there any other expression types that should be broken down
	// like this?
	switch (node.type) {
		case 'BinaryExpression':
		case 'LogicalExpression':
			return deblockBinaryOrLogicalExpression(node);
		case 'SequenceExpression':
			return deblockSequenceExpression(node);
		case 'ConditionalExpression':
			return deblockConditionalExpression(node);
		default:
			throw new Error('Unable to deblock node type "' + node.type + '"');
	}
}
...
function deblockBinaryOrLogicalExpression(node) {
	var tmpL = tmpVar(),
		tmpR = tmpVar(),
		left = node.left,
		right = node.right,
...
function deblockConditionalExpression(node) {
	var tmpT = tmpVar(),
		consequent = node.consequent,
		alternate = node.alternate,
		tAssign = protogen._createAssignmentExpression('=', tmpT, node.test);
	return protogen.createBlockExpression([
		toStatement(tAssign),
...
function deblockSequenceExpression(node) {
	var tmp = tmpVar(),
					...
					protogen.createBlockStatement([
						protogen._createExpressionStatement(
							protogen._createAssignmentExpression('=',
								tmp,
								protogen._createBlockExpression([
									node.consequent
								])
							)
						)
...
function BinaryExpression(node) {
	...
	if (containsBlockExpression(node))
		return deblockExpression(node);
	switch (node.operator) {
		case '::':
...
function BlockExpression(node) {

	// Check to see if block expression can be reduced to a sequence expression.
	if (isSequenceableBody(node.body))
		return bodyToSequenceExpression(node.body);

	var tmp = tmpVar(),
		block = protogen.createBlockStatement(node.body);
...
function CascadeStatement(node) {
	return protogen.createExpressionStatement(
		protogen._createCascadeExpression(node.object, node.body)
	);
}
...
function ForOfStatement(node, scope) {
	...
	&& (node.right.operator == '..' || node.right.operator == '...')
		...
		&& node.right.right.type == 'BinaryExpression'
		&& (node.right.right.operator == '..'
			|| node.right.right.operator == '...')))
