...
function isSequenceable(node) {
	if (node.type == 'ExpressionStatement')
		return !containsStatementExpression(node);
...
function containsStatementExpression(node) {
	var containsBlock = false;
	traverse(node, function(info) {
		var type = info.node && info.node.type;
		if (type == 'StatementExpression')
			containsBlock = true;
...
function assignCompletion(identifier, node) {
	...
	switch (node.type) {
		...
		case 'IfStatement':
			...
			tmp2 = tmpVar();
			return protogen.createBlockStatement([
				// TODO: Do places like this need to have something called on
				// them in case `node.test` contains a statement expression?
				esgen.createExpressionStatement(
...
function BinaryExpression(node) {
	...
	if (containsStatementExpression(node))
...
function StatementExpression(node) {
	...
	// TODO: This is old code from BlockExpression. Reimplement for StatementExpresion.
