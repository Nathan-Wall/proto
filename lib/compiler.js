...
function isSequenceableBody(body) {
	...
	for (var i = 0; i < body.length; i++) {
		...
		if (child.type != 'ExpressionStatement') {
			if (child.type == 'IfStatement' && isSequenceableIf(child))
				continue;
			if (child.type == 'BlockStatement' && isSequenceableBody(child))
				return true;
			return false;
		}
	}
...
function FunctionExpression(node, scope) {
			...
			body.body.unshift(
				protogen._createIfStatement(
					protogen._createBinaryExpression('==',
						node.params[i], esgen.createIdentifier('undefined')
					),
					protogen._createExpressionStatement(
						protogen._createAssignmentExpression(
	...
	if (node.fn_name === undefined)
		name = esgen.createIdentifier('undefined');
	else
	...
	if (async)
		...
		return toProtoCall('CreateFunction', [
			esgen.createIdentifier('undefined'), ret, name, arity, thisValue
		]);
