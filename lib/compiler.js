...
function addDefinedVar(id, forScope) {
	var scope, name;
	if (forScope)
...
function UnaryExpression(node) {
	switch(node.operator) {
		...
		case 'delete':
			...
			// TODO: Think about this some... it seems `delete foo();` is
			// is allowed by JS, whatever that means... Maybe some meaning
			// should be given to this form... or maybe it should raise an
			// exception in proprima.
			if (node.argument.autoBind)
				throw new SyntaxError('Unexpected auto-bind operator');
