...
function transpile(program, relPath, _idWhitelist) {
	...
	return {
		program: processed,
		needsGeneratorRuntime: needsGeneratorRuntime
	};
}
			...
			[
				protogen._createVariableDeclarator(
					...
					protogen._createFunctionExpression(
						...
						node.body, node.rest, node.generator, node.lexicalThis
					),
					null
				)
			],
...
function FunctionExpression(node, scope) {
	...
	var ret = esgen.createFunctionExpression(
		...
		body, null, node.generator, false
	);
	...
	if (node.generator) {
		needsGeneratorRuntime = true;
		ret = toProtoCall('CreateGenerator', [
			esgen.createIdentifier('undefined'),
			regenerator.transform(ret)
		]);
	}
	else {
...
function YieldExpression(node) {
	// This is transformed to ES5 via regenerator
	return esgen.createYieldExpression(node.argument, node.delegate);
}
