...
function process(node) {
	...
	var processor = processors[node.type],
		...
		oldNextScope = nextScope,
		keys;

	if (node.type == 'BlockStatement' || node.type == 'Program') {
		scope = create(currentScope);
		keys = Object.keys(nextScope);
		for (var i = 0; i < keys.length; i++)
			scope[keys[i]] = nextScope[keys[i]];
		nextScope = Object.create(null);
	}
	else
		scope = currentScope;
	...
	nextScope = oldNextScope;

	return processed;

}

function addDefinedVar(id, forNextScope) {
	var scope, name;
	if (forNextScope)
		scope = nextScope;
	else
	...
	scope['Identifier:defined_' + name] = true;
}
...
function BinaryExpression(node) {
	...
	switch (node.operator) {
		...
		case '-=':
		case '*=':
			return esgen.createBinaryExpression(node.operator,
...
function FunctionExpression(node, scope) {
	...
	addDefinedVar(node.id, true);

	// Prime transpiler to handle destructuring and add params to scope
	for (var i = 0; i < node.params.length; i++) {
		...
		addDefinedVar(node.params[i].id, true);
	}
