...
function transpile(program) {
	...
	traverse(program, function(info) {
		...
		|| node.operator == '|')
			throw new Error('Operators not supported: ^ and |');
		else if (node.type == 'UnaryExpression'
		&& node.operator == '#') {
	...
	function UnspreadArgs(nodes) {
		return toProtoCall(
			'ArrayMerge',
			nodes.map(function(node) {
				if (node.type == 'SpreadElement')
					return toProtoCall('CheckSpread', [ node.argument ]);
				return ArrayWrap([ node ]);
			})
		);
	}
	...
	function replaceCallExpression(node) {
		...
		if (node.callee.type == 'MemberExpression') {
			...
			return new Replacement(
				toProtoCall(own ? 'CallOwnMethod' : 'CallMethod',
					[ obj, prop, UnspreadArgs(node.arguments) ]
				)
			);
		}
		...
		return new Replacement(
			toProtoCall('Call',
				[ fn, obj, UnspreadArgs(node.arguments) ]
			)
		);
	}
