...
prevScope['sym_@iterator'] = toProtoProperty('@iterator');
prevScope['sym_@toComparable'] = toProtoProperty('@toComparable');
...
function toProtoCall(fn, args) {
	return esgen.createCallExpression(toProtoProperty(fn), args);
}
...
function ForOfStatement(node, scope) {
	...
	scope['Identifier_' + left.name] = u.name;
	body = process(body);
	delete scope['Identifier_' + left.name];
