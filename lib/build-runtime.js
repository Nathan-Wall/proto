...
// If neededProps is undefined, the full runtime will be included.
function build(neededProps) {

	var info, defs, propNames;

	if (neededProps === undefined) {
		info = includeAll();
		defs = info.defs;
		propNames = info.propNames;
	}
	else {
		info = includeNeeded(neededProps);
		defs = info.defs;
		propNames = info.propNames;
	}
	...
	defs.push(
		esgen.createReturnStatement(
			esgen.createCallExpression(
				...
				[ esgen.createCallExpression(
					esgen.createIdentifier('own'),
					[ toObjectLiteral(propNames) ]
				) ]
			)
		)
	);
...
function includeNeeded(neededProps) {

	var propsLists = createPropsLists(),
		vars = propsLists.vars,
		funcs = propsLists.funcs,
		usedInfo = addNeededProps(neededProps, vars, funcs, [ 'own' ]),
	...
	return { defs: defs, propNames: neededPropsNames };
...
function includeAll() {
	...
	// TODO: It might be possible to make includeNeeded faster by just keeping
	// dependencies in order based on the kind of ordering being done below,
	// so that includeNeeded doesn't have to determine order... ?

	// Move bootstrap.js to the front.
	var index = files.indexOf('bootstrap.js');
	if (!~index)
		throw new Error('Expected "bootstrap.js" file');
	files.splice(index, 1);
	files.unshift('bootstrap.js');

	// Move globals.js to the back.
	index = files.indexOf('globals.js');
	if (!~index)
		throw new Error('Expected "globals.js" file');
	files.splice(index, 1);
	files.push('globals.js');

	// Move poststrap.js to the back.
	index = files.indexOf('poststrap.js');
	if (!~index)
		throw new Error('Expected "poststrap.js" file');
	files.splice(index, 1);
	files.push('poststrap.js');
			...
			switch(node.type) {
				case 'VariableDeclaration':
					...
					for (var k = 0, d; k < node.declarations.length; k++)
						propNames.push(node.declarations[k].id.name);
					break;
				case 'FunctionDeclaration':
					defs.push(node);
					propNames.push(node.id.name);
					break;
				case 'EmptyStatement':
					break;
				default:
	...
	return { defs: defs, propNames: propNames };
...
function addNeededProps(neededProps, vars, funcs, required) {
	...
	var names = [ ],
		...
		prop, deps, end = [ ], d, i, j, found;

	for (i = 0; i < required.length; i++) {
		prop = required[i];
		found = false;
		for (j = 0; j < neededProps.length; j++)
			if (neededProps[j].name == prop) {
				found = true;
				break;
			}
		if (!found)
			neededProps.push({ name: prop });
	}

	for (i = 0; i < neededProps.length; i++) {
		prop = neededProps[i];
		names.push(prop.name);
		deps = getDependencies(prop).deps.slice();
	...
	function getDependencies(prop, seen) {
		...
		if (forDepsMap)
			depsMap[propName] = { deps: deps, cycle: cycle };
